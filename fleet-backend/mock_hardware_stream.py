...
async def main():
    ...
    client.publish(MQTT_TOPIC, generate_signed_payload(
        start_payload, HMAC_SECRET))
    ...
    print(
        "🚧 [Event 3/4] เบรกกะทันหันอย่างรุนแรงในเขตก่อสร้าง (ความเร็วต่ำ 15 กม./ชม.)..."
    )
    ...
    stop_payload.update({"ts": base_ts + 30, "speed": 0.0,
                        "rpm": 0, "ignition": False})
    client.publish(MQTT_TOPIC, generate_signed_payload(
        stop_payload, HMAC_SECRET))
